...
interface LoginProps {
    onLogin: (token: string, user: string, warehouseAccess?: string) => void;
}
...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.data.status === 'success') {
                const { token, user, warehouse_access } = response.data;
                if (rememberMe) {
                ...
                onLogin(token, user, warehouse_access);
            }
        } catch (err: any) {
    ...
    return (
        <div translate="no" className="notranslate min-h-screen w-full bg-[#020617] flex items-center justify-center p-4 overflow-hidden relative">
